...
function ToolCallCard({ block }: { block: Extract<UIContentBlock, { type: "toolCall" }> }) {
  ...
  return (
    <details className="my-1.5 rounded-lg border border-neutral-800 bg-neutral-900/60 text-sm">
      <summary className="flex cursor-pointer items-center gap-2 px-3 py-2 select-none">
        ...
        />
        <span className="font-medium text-neutral-300">{block.name}</span>
      ...
      <div className="border-t border-neutral-800 px-3 py-2">
        ...
        {block.result && (
          <pre
            className={`mt-2 max-h-64 overflow-auto border-t border-neutral-800 pt-2 font-mono text-xs whitespace-pre-wrap ${
              block.result.isError ? "text-red-400" : "text-neutral-300"
            }`}
          >
...
function Thinking({ text }: { text: string }) {
  return (
    <details className="my-1.5 text-sm">
      ...
      <div className="mt-1 border-l-2 border-neutral-800 pl-3 text-neutral-500 italic whitespace-pre-wrap">
        {text}
      </div>
    </details>
  );
...
function Blocks({ blocks }: { blocks: UIContentBlock[] }) {
  return (
    <>
      {blocks.map((b, i) => {
        switch (b.type) {
  ...
  if (message.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] rounded-2xl rounded-br-md bg-indigo-600 px-4 py-2.5 text-[15px] whitespace-pre-wrap sm:max-w-[75%]">
  ...
  return (
    <div className="text-[15px] text-neutral-100">
      ...
      {message.errorMessage && (
        <div className="mt-2 rounded-lg border border-red-900 bg-red-950/50 px-3 py-2 text-sm text-red-400">
          {message.errorMessage}
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-3xl flex-col gap-5 px-4 py-6">
        ...
        {streamText && (
          <div className="text-[15px] whitespace-pre-wrap leading-relaxed text-neutral-100">
            {streamText}
          </div>
        )}
        {activeTools.map((t) => (
